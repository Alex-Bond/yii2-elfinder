...
class elFinder
{
                        ...
                        if ($handler) {
                            if (is_string($handler) && strpos($handler, '.')) {
                                ...
                                if (strcasecmp($_domain, 'plugin') === 0) {
                                    if ($plugin = $this->getPluginInstance($_name,
    ...
    {
        return isset($_SESSION[$this->netVolumesSessionKey]) && is_array($_SESSION[$this->netVolumesSessionKey]) ? $_SESSION[$this->netVolumesSessionKey] : [];
    }
    ...
    protected function netmount($args)
    {
        ...
        if ($protocol === 'netunmount') {
            ...
            return ['error' => $this->error(self::ERROR_NETUNMOUNT)];
        }
                ...
                foreach ($args['upload'] as $i => $url) {
                    ...
                    if ($args['chunk']) {
                        if ($url === 'chunkfail' && $args['mimes'] === 'chunkfail') {
                            ...
                            if (preg_match('/^(.+)(\.\d+_(\d+))\.part$/s', $chunk, $m)) {
                                $result['warning'] = $this->error(self::ERROR_UPLOAD_FILE, $m[1],
                    ...
                    if ($data) {
                        $_name = isset($args['name'][$i]) ? $args['name'][$i] : preg_replace('~^.*?([^/#?]+)(?:\?.*)?(?:#.*)?$~',
                            '$1', rawurldecode($url));
                        if ($_name) {
                            $_ext = '';
